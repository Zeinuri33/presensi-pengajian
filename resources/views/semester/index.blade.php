    ...
    <div class="card-header align-items-center py-5 gap-2 gap-md-5" style="background-image: url('{{ asset('assets/media/ibrahimy/akses.png') }}'); background-size: cover; background-position: center; background-color: rgba(0,0,0,0.3); background-blend-mode: darken; color: white;">
        ...
        <div class="card-title">
            ...
            <div class="d-flex align-items-center position-relative my-1">
                ...
                <input type="text" data-kt-ecommerce-product-filter="search" class="form-control form-control-sm w-250px ps-12" placeholder="Cari Semester" />
            </div>
            <!--end::Search-->
        </div>
        <!--end::Card title-->
        ...
        <table class="table align-middle table-striped fs-6 gy-4" id="kt_ecommerce_products_table">
            <thead class="fw-bold fs-5 bg-primary">
                <tr class="text-start text-white fw-bold fs-7 text-uppercase gs-0">
                    <th class="min-w-200px rounded-start ps-4">Semester</th>
                    <th>Tahun Ajaran</th>
                    <th>Dari Tanggal</th>
            ...
            <tbody class="fw-semibold text-gray-600">
                ...
                <tr>
                    <td class="ps-4 text-dark">Semester {{ $item->semester }}</td>
                    <td>{{ $item->tapel }}</td>
                    <td>
                        <div class="badge badge py-3 px-4 fs-7 badge-light-info">{{ Carbon\Carbon::parse($item->dari_tgl)->translatedFormat('d F Y') }}</div>
                    </td>
                    <td>
                        <div class="badge badge py-3 px-4 fs-7 badge-light-warning">{{ Carbon\Carbon::parse($item->sampai_tgl)->translatedFormat('d F Y') }}</div>
                    </td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td class="text-end pe-2">
                        ...
                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
                            ...
                            @can('semester-hapus')
                            <div class="menu-item px-3">
                                <a href="{{ asset('semester/'.$item->id.'/hapus') }}" class="menu-link px-3 delete-button">Hapus</a>
                            </div>
                            @endcan
                            <!--end::Menu item-->
                        </div>
